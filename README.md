...
Требуется [golang-migrate CLI](https://github.com/golang-migrate/migrate/tree/master/cmd/migrate#installation):

```bash
go install -tags 'postgres' github.com/golang-migrate/migrate/v4/cmd/migrate@latest
```

Применить миграции (`?sslmode=disable` для локальной БД без SSL):
